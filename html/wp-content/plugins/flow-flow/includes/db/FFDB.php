...
class FFDB {
	...
	public static function streams($table_name){
		if (false !== ($result = self::conn()->getIndCol('id', 'SELECT `id`, `name`, `layout`, `value` FROM ?n ORDER BY `id`',
				$table_name))){
			return $result;
		}
		return array();
	...
	public static function getStatusInfo($cache_table_name, $streamId, $format = true) {
		...
		$status_info = FFDB::conn()->getAll('select `src`.`stream_id` as `id`, MIN(`cach`.`status`) as `status`, COUNT(`cach`.`feed_id`) as `feeds_count` from ?n `cach` inner join ?n `src` on `cach`.`feed_id` = `src`.`feed_id`  ?p  group by `src`.`stream_id`',
			$cache_table_name, $streams_sources_table_name, $sql_part);
	...
	public static function getError($cache_table_name, $streamId, $format = true){
		...
		$result = '';
		$errors = FFDB::conn()->getInd('feed_id', 'select `cach`.`errors`, `cach`.`feed_id` from ?n `cach` inner join ?n `src` on `cach`.`feed_id` = `src`.`feed_id` where `src`.`stream_id` = ?s', $cache_table_name, $streams_sources_table_name, $streamId);
